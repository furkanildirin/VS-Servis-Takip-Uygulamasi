...
import 'package:path_provider/path_provider.dart'; // Platform bağımsız dosya yolları için
...
class DBHelper {
  ...
  // Veritabanını sıfırlamadan önce yedekleme fonksiyonu
  ...
  Future<void> resetDatabase() async {
    await backupDatabase(); // Sıfırlamadan önce yedekle
  ...
  Future<Database> _initDatabase() async {
    ...
    return openDatabase(
      path,
      version: 6, // Veritabanı versiyonu
  ...
  Future<void> insertDevice(Map<String, dynamic> device) async {
    try {
      ...
      device['technician'] = device['technician'] ?? ''; // Boş alanı boş string olarak ayarla

      await db.insert(
        'devices',
        device,
        conflictAlgorithm: ConflictAlgorithm.ignore,  // Aynı servis numarasını engelle
      );
    } catch (e) {
  ...
  Future<void> markDeviceAsCompleted(int id) async {
    try {
      ...
      // Cihazı tamamlananlar listesine ekleyip, aktif listeden silme
      // await deleteDevice(id); // Bu satırı kaldırdık
    } catch (e) {
  ...
  Future<void> exportRegisteredDevicesToExcel () async {
    try {
      final db = await database;
      final devices = await db.query('devices');

      var excel = Excel.createExcel();
      Sheet sheetObject = excel['Devices'];
      ...
      for (var device in devices) {
        sheetObject.appendRow([
          ...
          device['invoiceDate'],
          device['warrantyInfo'],
          device['overallCondition'],
          device['serviceArrivalDate'],
          device['totalFee'],
          device['explanation'],
          device['accompanyingItems'],
          device['damageDescription'],
          device['imagePath'],
          device['isExternalService'] == 1 ? 'Yes' : 'No',
          device['isCompleted'] == 1 ? 'Yes' : 'No',
          device['technician'],
        ]);
      }
      ...
      print('Error exporting devices to Excel: $e');
    }
  }
}
